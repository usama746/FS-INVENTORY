...
function cacheDOMElements() {
   ...
   const criticalElements = ["mainContent", "toast", "modalOverlay"];
   const missingElements = criticalElements.filter((id) => !DOM[id]);
}
...
function updateRefreshButtonVisibility() {
   ...
   if (refreshBtn) {
      // Always show refresh button for easy access
          ...
          ${
             !isAdmin
                ? `<button onclick="window.togglePriceVisibility('${
                     ...
                     itemPriceVisibility[item.id] ? "👁️ Show" : "🙈 Hide"
                  }</button>`
                : ""
          }
        </div>
...
function startAddVariant(itemId) {
   ...
   if (!item) return;
   const cards = DOM.mainContent?.querySelectorAll(".card");
   let targetCard = null;
